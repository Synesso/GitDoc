...
- [x] Create `middleware.ts` — optimistic auth gate on `/api/repos/*` that short-circuits with 401 if session cookie is absent
...
- [x] Create `POST /api/repos/[owner]/[repo]/pulls/[pull_number]/comments` — creates a new PR review comment (maps camelCase body to GitHub's snake_case, adds `side: "RIGHT"`)
- [x] Create `POST /api/repos/[owner]/[repo]/pulls/[pull_number]/comments/[comment_id]/replies` — replies to an existing comment thread
- [x] Create `GET /api/repos/[owner]/[repo]/pulls/[pull_number]/head` — lightweight endpoint returning only headSha + state for stale-SHA polling
- [x] Create `GET /api/repos/[owner]/[repo]/contents/[...path]` — fetches file content at a specific ref, decodes base64 to UTF-8; doubles as image proxy for private repos (streams raw bytes with correct Content-Type for image extensions)
- [x] Handle pagination for files and comments — GitHub caps at 100 per page; aggregate all pages before returning
...
- [x] Create `POST /api/repos/[owner]/[repo]/pulls/[pull_number]/threads/[threadId]/unresolve` — calls `unresolveReviewThread` GraphQL mutation

## Markdown Rendering Pipeline

- [x] Install `react-markdown`, `remark-gfm`, `remark-rehype` — core markdown rendering with GFM support (tables, task lists, strikethrough, alerts)
...
- [x] Write `makeUrlTransform()` function — rewrites relative image paths to `raw.githubusercontent.com/{owner}/{repo}/{headSha}/...` and relative `.md` links to GitHub blob view
...
- [x] Build `getCommentableLines(file)` function — extracts commentable line numbers from parsed diff (`add` → `ln`, `normal` → `ln2`), returns `Set<number>` or read-only status with reason
- [x] Handle absent `patch` field — detect large diffs, binary files, renamed-with-no-changes; mark file as read-only with appropriate reason string
- [x] Mark rendered elements with `data-commentable` attribute — cross-reference each element's `[sourceStart, sourceEnd]` range against the `commentableLines` set during rendering
...
- [x] Implement click-to-scroll sync — clicking a thread card scrolls the passage into view (`scrollIntoView({ behavior: 'smooth', block: 'center' })`), and vice versa
- [x] Display resolved threads collapsed/dimmed with "Resolved" badge and `resolvedBy` avatar — expand on click to see history
- [x] Show resolve/unresolve buttons conditionally based on `viewerCanResolve`/`viewerCanUnresolve` — calls GraphQL mutations with optimistic UI
- [x] Display outdated threads in a separate "Outdated Comments" section at sidebar bottom — collapsed by default, dimmed (`opacity: 0.6`), "Outdated" badge, original `diff_hunk` shown as code block on expand
...
- [x] Wrap rendered markdown in `<article className="prose dark:prose-invert lg:prose-lg max-w-none">` — typography plugin styles with dark mode support
- [x] Build mobile layout (<768px) with bottom Drawer (Vaul) — full-width content, floating comment count badge, drawer with `CommentThreadList` inside `ScrollArea`
- [x] Build `useMediaQuery` hook — switches between desktop aside and mobile drawer based on `(min-width: 768px)`
- [x] Add inline comment indicators in left gutter on mobile — tapping opens drawer pre-scrolled to that thread
...
- [x] Show stale-SHA banner — non-intrusive top-of-page banner with "Refresh Now" and "Dismiss" when `isStale` is true
- [x] Implement re-sync flow on "Refresh Now" — save comment drafts, re-fetch file content + diff at new SHA, rebuild `commentableLines`, re-render markdown, restore drafts, update stored headSha
- [x] Preserve scroll position on refresh — record `data-source-start` of element at viewport top before refresh, `scrollIntoView()` the same element after re-render
